...
export async function pricingAdminPage(env){
  ...
  ${field("processingFee","ROVIQ processing / coordination fee",p.processingFee,"USD")}
...
export async function savePricing(request,env){
  ...
  const input={};
  for(const key of ["processingFee","shippingLow","shippingHigh"]) input[key]=form.get(key);
