...
.uk-motion{position:relative;height:92px;margin:18px 0 8px;overflow:hidden;border-radius:14px;background:linear-gradient(180deg,rgba(7,19,31,.1),rgba(8,24,36,.72));border:1px solid rgba(200,146,69,.18)}
.uk-motion::before{content:"";position:absolute;left:-15%;right:-15%;bottom:21px;height:2px;background:linear-gradient(90deg,transparent,rgba(200,146,69,.2),rgba(200,146,69,.85),rgba(200,146,69,.2),transparent);box-shadow:0 0 16px rgba(200,146,69,.45);animation:ukRoadPulse 3.8s ease-in-out infinite}
.uk-motion::after{content:"";position:absolute;inset:0;background:repeating-linear-gradient(100deg,transparent 0 90px,rgba(96,165,250,.05) 90px 92px);transform:translateX(-120px);animation:ukGridMove 7s linear infinite}
.uk-car{position:absolute;left:-140px;bottom:24px;width:118px;height:34px;border-radius:24px 34px 10px 10px;background:linear-gradient(180deg,#dca950,#8d5f20);box-shadow:0 0 24px rgba(220,169,80,.34);animation:ukCarDrive 8.5s cubic-bezier(.4,0,.2,1) infinite;z-index:2}
.uk-car::before{content:"";position:absolute;left:28px;top:-19px;width:58px;height:22px;border-radius:26px 30px 4px 4px;background:linear-gradient(180deg,#173448,#0c2231);border:1px solid rgba(220,169,80,.65);transform:skewX(-16deg)}
.uk-car::after{content:"";position:absolute;left:8px;right:8px;bottom:-8px;height:14px;background:radial-gradient(circle at 18px 7px,#05080b 0 6px,#788693 6.5px 8px,transparent 8.5px),radial-gradient(circle at calc(100% - 18px) 7px,#05080b 0 6px,#788693 6.5px 8px,transparent 8.5px)}
.uk-light{position:absolute;left:-22%;top:21px;width:28%;height:3px;background:linear-gradient(90deg,transparent,rgba(74,161,255,.9),rgba(255,255,255,.9),transparent);filter:blur(.4px);opacity:.75;animation:ukLightSweep 5.5s linear infinite}
@keyframes ukCarDrive{0%{transform:translateX(0) scale(.92);opacity:0}8%{opacity:1}48%{transform:translateX(calc(50vw + 120px)) scale(1)}88%{opacity:1}100%{transform:translateX(calc(100vw + 320px)) scale(.96);opacity:0}}
@keyframes ukLightSweep{0%{transform:translateX(0);opacity:0}12%{opacity:.8}100%{transform:translateX(155vw);opacity:0}}
@keyframes ukRoadPulse{0%,100%{opacity:.45}50%{opacity:1}}
@keyframes ukGridMove{to{transform:translateX(120px)}}
