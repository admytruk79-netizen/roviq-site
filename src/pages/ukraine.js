...
function inventorySections(vehicles){
  const makes=["Chevrolet","GMC","Ford"];
  return makes.map(make=>{
    const matching=vehicles.filter(v=>v.make===make);
    if(!matching.length) return "";
    const models=[...new Set(matching.map(v=>v.model||"Other trucks"))]
      .sort((a,b)=>a.localeCompare(b,undefined,{numeric:true}));
    return `<section class="uk-make-section" id="make-${make.toLowerCase()}"><div class="uk-section-heading"><h2>${make}</h2><span>${matching.length} vehicles</span></div>${models.map(model=>{
...
const makeCounts=Object.fromEntries(["Chevrolet","GMC","Ford"].map(make=>[make,vehicles.filter(v=>v.make===make).length]));
